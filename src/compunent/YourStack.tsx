...
function  YourStack({stack, onRemove, onRemoveAll}: {stack:CardType[]; onRemove: (id: string) => void;}){
   return(
    <div className="border-gray-200 rounded-xl p-5 sticky top-5">
        <h2>Your Stack</h2>
        {stack.length === 0 ? (
            <div>
                <p className="taxt-gray-500">No Technology Selected</p>
        ...
        <p className="text-gray-500">{stack.length} Technology Selected</p>
                {stack.map((item) => (
                    <div key={item.id} className="flex items-center justify-between py-2 border-b border-gray-200">
                        <img src={item.icon} alt={item.name} className="h-30px w-30px" />
                        <div>
